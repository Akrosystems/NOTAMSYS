...
- **AFTN/Comsoft, GCAA website and email publication channels stay simulated by default** (`NOTAMSYS_PUBLICATION_MODE=simulated_sync`) -- there's no live circuit or CMS credentials to point at yet. AFTN and Email specifically now have a real path once credentials exist:
